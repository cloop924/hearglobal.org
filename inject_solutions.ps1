...
$explanations = @{
    ...
    "2" = @{
        ...
        "6" = "The text says 'Tap the number of words in this sentence.' There are 8 words. Ignore the lightbulbs!"
    ...
    "3" = @{
        ...
        "6" = "The prompt simply says to tap 'more than three times'. Since the target threshold is 3, tapping anything above 3 passes the system test."
    ...
    "4" = @{
        ...
        "3" = "Info round establishing a completely new persistent rule: 'If the answer is exactly three, do not tap'. Remember this!"
    ...
    "5" = @{
        ...
        "2" = "Count the objects: there are 3 shapes total. No wildcards or rules yet, so the answer is 3."
    ...
    "6" = @{
        ...
        "5" = "The prompt says 'Count all shapes'. There is 1 red circle and 1 green circle. That's 2 shapes total."
...
$jsonStr = Get-Content -Path $solutionsPath -Raw
$solutions = $jsonStr | ConvertFrom-Json

foreach ($lvl in $solutions) {
    $lstr = [string]$lvl.levelId
    if ($explanations.ContainsKey($lstr)) {
        foreach ($round in $lvl.rounds) {
            $rstr = [string]$round.roundId
            if ($explanations[$lstr].ContainsKey($rstr)) {
                $round.explanation = $explanations[$lstr][$rstr]
            }
        }
    }
}

$solutions | ConvertTo-Json -Depth 10 | Set-Content -Path $solutionsPath
Write-Host "Successfully injected manual explanations for levels 1-10 via PowerShell."
