...
const levelsDir = 'c:/Users/pc/Desktop/gamedev/tapgame/assets/levels';
const outputPath = 'c:/Users/pc/Desktop/HearGlobal/website/solutions.json';
...
const wildcards = {
    ...
    25: { name: "Engraved Brass Coin", effect: "The Engraved Brass Coin is active. If this is the final round (Round 10), the target is 0." }
};
    ...
    if (file.endsWith('.json')) {
        ...
        for (let round of data.rounds) {
            let roundObj = {
                ...
                prompt: round.prompt,
                wildcardId: round.wildcardId || 0,
                wildcardName: wildcards[round.wildcardId || 0]?.name || "Unknown",
                target: round.target != null ? round.target : null,
                isInfo: round.isInfo || false,
                icons: []
            };

            if (round.layoutElements) {
                for (let el of round.layoutElements) {
            ...
            if (round.isInfo) {
                ...
                let effectText = wildcards[roundObj.wildcardId]?.effect || "A mysterious wildcard changes the rules.";
                if (roundObj.wildcardId === 0) {
                    explanation = `Based simply on reading the prompt, the correct answer is ${round.target} taps. No wildcards are active to alter this outcome.`;
                } else {
                    explanation = `The prompt is trying to trick you or guide you to a different number. However, you must remember the active wildcard rule! ${effectText} Applying this rule leads to the final required tap count of ${round.target}.`;
                }
            }

            roundObj.explanation = explanation;
            solutionsLevel.rounds.push(roundObj);
        }

        allLevels.push(solutionsLevel);
    }
}

// Sort by levelId so they appear in order
allLevels.sort((a, b) => a.levelId - b.levelId);

fs.writeFileSync(outputPath, JSON.stringify(allLevels, null, 2));
console.log("Successfully generated solutions.json for " + allLevels.length + " levels.");
